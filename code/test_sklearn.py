...
class Runner:
    ...
    def init_sets(self, sc, file_path, num_clusters):
        ...
        c2_points_x = []
        c2_points_y = []
        print("Labels len: {}".format(len(clusters.labels)))
        for idx, point in enumerate(points_sample):
            label = clusters.labels[idx]
            if label == 0:
                c1_points_x.append(point[0])
                c1_points_y.append(point[1])
            if label == 1:
                c2_points_x.append(point[0])
                c2_points_y.append(point[1])
        plt.scatter(c1_points_x, c1_points_y, color='red')
        plt.scatter(c2_points_x, c2_points_y, color='blue')
        plt.show()
